...
socket.api_v1(({ tourney }) => {
    try {
        ...
        if (
            ...
            getTeamFullInfoByName(tourney.manager.teamName.left).then((leftTeam) => {
                // 设置队伍头像、名称
                document.getElementById("team-a-name").innerText = leftTeam.FullName;
                document.getElementById("team-a-avatar").src =
                    "../COMMON/img/flag/" + leftTeam.Acronym + ".png";
                // 设置队伍成员
            ...
            getTeamFullInfoByName(tourney.manager.teamName.right).then(
                (rightTeam) => {
                    ...
                    document.getElementById("team-b-avatar").src =
                        "../COMMON/img/flag/" + rightTeam.Acronym + ".png";
...
function restoreBeatmapSelection() {
    ...
    if (beatmapSelectionMap != null) {
        ...
        if (teamAContainer && teamBContainer) {
            ...
            // 使用Promise.all处理异步操作
            Promise.all(
                beatmapSelections.map((beatmapOperation) => {
                ...
                .then((results) => {
                    results.forEach(([beatmap, mods, beatmapOperation]) => {
                        ...
                        let operationContainer =
                            team === TEAM_RED ? teamAContainer : teamBContainer;
                        let operation = document.createElement("div");
                        // 离线图的改动恰好兼容了空ban，这里找不到对应MOD也不会出现异常，而是直接插入空白div
                        if (type === "Blank") {
                            operation.id =
                                team === TEAM_RED ? "team-a-blank" : "team-b-blank";
                            operationContainer.appendChild(operation);
                        } else {
                            operation.id = beatmap.ID;
                            if (type === "Pick") {
                                operation.classList.add(
                                    team === TEAM_RED ? "team-a-pick" : "team-b-pick",
                                );
                            }
                            if (type === "Ban") {
                                operation.classList.add(
                                    team === TEAM_RED ? "team-a-ban" : "team-b-ban",
                                );
                            }
